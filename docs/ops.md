...
6. **Update the Summary Report**:
   - Use the built-in helper: Pass tags to `updateSummaryMarkdown` in [src/reporting/summary.ts](https://github.com/salman-chowdhury/next-speed-kit/blob/main/src/reporting/summary.ts).
...
- **Throttling and Flags**: Customize via audit options in [src/audit/lighthouse.ts](https://github.com/salman-chowdhury/next-speed-kit/blob/main/src/audit/lighthouse.ts), e.g., mobile vs. desktop emulation.
...
- **New Codemods**: Add transforms in `src/codemods/transforms/` and register in [src/codemods/index.ts](https://github.com/salman-chowdhury/next-speed-kit/blob/main/src/codemods/index.ts). Follow patterns like `next-image-dimensions`.
- **Bundle Analysis**: Enhance scanning of emitted assets; integrate Webpack stats for deeper insights.
- **Audit Customizations**: Modify `runLighthouseAudit` in [src/audit/lighthouse.ts](https://github.com/salman-chowdhury/next-speed-kit/blob/main/src/audit/lighthouse.ts) for environment-specific options (e.g., throttling, device emulation).
